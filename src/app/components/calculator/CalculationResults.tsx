...
export default function CalculationResults({
  ...
  return (
    <div className="border-t border-gray-800 pt-6 space-y-6">
            ...
            <>
              {multiple >= 100 ? (
                <span className="text-red-500">
                  🤯 미쳤다... {multiple.toFixed(0)}배...
                </span>
              ) : multiple >= 10 ? (
                <span className="text-orange-500">
                  😱 헐... {multiple.toFixed(1)}배나...
                </span>
              ) : multiple >= 2 ? (
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-gray-800/30 rounded-lg p-4">
          <div className="flex items-center justify-between">
            ...
            <span
              className={`text-xl font-bold ${multiple >= 2 ? 'text-yellow-500' : 'text-gray-300'}`}
            >
        ...
        <div className="bg-gray-800/30 rounded-lg p-4">
          <div className="flex items-center justify-between">
            ...
            <span
              className={`text-xl font-bold ${calculation.yearlyReturn >= 0 ? 'text-green-500' : 'text-red-500'}`}
            >
              {calculation.yearlyReturn >= 0 ? '+' : ''}
              {calculation.yearlyReturn.toFixed(2)}%
            </span>
          </div>
        </div>
        ...
        <div className="bg-gray-800/30 rounded-lg p-4">
          <div className="flex items-center justify-between">
            ...
            <span
              className={`text-xl font-bold ${calculation.currentPrice >= calculation.pastPrice ? 'text-green-500' : 'text-red-500'}`}
            >
              {(
                (calculation.currentPrice / calculation.pastPrice - 1) *
                100
              ).toFixed(2)}
              %
            </span>
      ...
      {isProfit && (
        <div
          className="relative overflow-hidden rounded-lg bg-gradient-to-r from-yellow-500/10 to-orange-500/10
            ...
            {multiple >= 100 ? (
              <>
                ...
                <p className="text-yellow-500 font-medium text-center text-lg">
                  {formatCurrency(calculation.investAmount)}원 투자했으면...
                </p>
                <p className="text-white text-center text-2xl lg:text-3xl font-bold mt-2">
                  {formatCurrency(calculation.currentValue)}원 됐을텐데!
                </p>
              ...
              <>
                ...
                <p className="text-white text-center text-2xl lg:text-3xl font-bold mt-2">
                  {formatCurrency(calculation.profit)}원 벌었을텐데!
                </p>
              </>
            ) : (
              <>
      ...
      {!isProfit && (
        <div className="rounded-lg bg-blue-500/10 border border-blue-500/30 p-4">
          <p className="text-blue-400 text-center">
            {formatCurrency(Math.abs(calculation.profit))}원 손실을 피했습니다!
          </p>
        </div>
      )}
    </div>
...
function ResultCard({
  ...
  return (
    <div className="bg-gray-800/30 rounded-lg p-4 hover:bg-gray-800/50 transition-colors">
      ...
      <p
        className={`${valueColor} ${isBold ? 'font-bold text-lg lg:text-xl' : 'font-medium'}`}
      >
        {value}
      </p>
