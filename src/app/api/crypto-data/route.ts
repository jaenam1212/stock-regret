...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const ohlcResponse = await fetch(ohlcUrl, { cache: 'no-store' });
    if (!ohlcResponse.ok) {
    ...
    const volumeResponse = await fetch(volumeUrl, { cache: 'no-store' });
    let volumeData: any[] = [];
    ...
    // OHLCV 데이터 포맷팅
    const data = ohlcData.map((ohlc: [number, number, number, number, number], index: number) => {
    ...
    const currentPrice = priceData[coinId]?.usd || data[data.length - 1]?.close || 0;
    const change24h = priceData[coinId]?.usd_24h_change || 0;

    // 전날 가격 계산
    const previousPrice = data[data.length - 2]?.close || currentPrice;
    const change = currentPrice - previousPrice;
    const changePercent = previousPrice > 0 ? (change / previousPrice) * 100 : change24h;
    ...
    return NextResponse.json(
      {
        ...
        suggestion: '지원하는 암호화폐인지 확인하세요 (예: bitcoin, BTC, 비트코인)',
      },
      { status: 500 }
    );
  }
